...
model_sel = ["mobilenetv2"]
# model dict: [layer number, GMACs]
model_dict = {"mobilenetv2": [53, 0.319]}
hw_utl = [38.08, 214] # [kLUT, DSP]

parser = argparse.ArgumentParser()
parser.add_argument("-m", "--model", type=str, default='mobilenetv2')
dnn_model = parser.parse_args().model
model_csv = "xc7z020_" + dnn_model + ".csv"

assert dnn_model in model_sel, "This script is only for mobilenetv2!"
...
for test_idx in range(10):
    ...
    # get data
    latency_ms_1 = ac.report_latency(100)
    ...
    ig.set_weight_addr(weight_array.physical_address)
    ig.set_act_in_addr(act_in_array.physical_address)
    ig.gen_instr(hw_arch, "xc7z020_mobilenetv2_2.csv", layer_num, instr_array)
    ...
    # get data
    latency_ms_2 = ac.report_latency(100)
    
    latency_ms = latency_ms_1 + latency_ms_2
